...
async function girlssearch({ firstName, lastName }) {
    ...
    let data = await (async () => {
        ...
        if (firstName || lastName) {
            if (firstName) {
                await page.goto(`https://www.ajga.org/rankings/girls?q=${firstName}`, { waitUntil: "domcontentloaded" })
            } else if (lastName) {
                await page.goto(`https://www.ajga.org/rankings/girls?q=${lastName}`, { waitUntil: "domcontentloaded" })
            }
